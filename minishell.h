...
/*   Updated: 2025/08/02 14:11:23 by kkuramot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
pid_t							execute_pipeline_loop(t_cmd *cmds,
...
char							*get_cd_target(char **args, char **envp,
									int *needs_free);
void							execute_pwd(void);
void							execute_export(char **args, t_shell *shell);
void							handle_export_no_value(char *arg,
									t_shell *shell);
...
void							handle_word_token(t_token *token, t_cmd *curr,
									t_shell *shell);
int								handle_redir_token(t_token **tokens_ptr,
									t_cmd *curr, t_shell *shell);
...
char							*expand_variables(const char *str,
									t_shell *shell);
void							handle_quotes(char c, char *quote_state,
									char **result);
void							append_char_to_result(char **result, char c);
void							append_str_to_result(char **result,
									const char *s);
...
void							parse_redirection(char **s, t_cmd *cmd,
									t_shell *shell);
void							assign_fd_to_cmd(int fd, const char *op,
									t_cmd *cmd);

char							**copy_envp(char **envp);
void							free_envp(char **envp);
char							*get_env_value(const char *name, char **envp);
char							**overwrite_env(const char *key,
...
int								parse_redirection_token(t_token *op,
									t_token *file, t_cmd *cmd, t_shell *shell);
...
int								validate_syntax_tokens(t_token *tokens,
									t_shell *shell);
void							initialize_shell(t_shell *shell, char **envp);
char							*join_lines_for_quotes(char *line,
									char *newline);
int								is_quoted_whitespace(const char *str);
char							*expand_tilde_in_line(char *line,
									t_shell *shell);
